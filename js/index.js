let accountsFeed = 'https://rotterdam.tcbmedia.eu/demo/accounts/';
let dataRequest = new XMLHttpRequest();
let whenDataLoaded = function () {
  let dataText = dataRequest.responseText;
  dataObject = JSON.parse(dataText);
  let accounts = dataObject['accounts'];
  let accountDisplay = $('#app');
  accountDisplay.html("");

  for (i = 0; i < accounts.length; i++) {

    if (accounts[i]['typeAccount'] == 'Checking') {
      $('<div class="col-3 accountDisplay zoomInDownAnim"><div class="row "><div class="col-3 iconLeft"><span><i class="fas fa-users fa-lg"></i></span></div><div class="col iconIng"><h5>ING</h5><img class="inglion" src="https://i.pinimg.com/originals/ff/17/ab/ff17ab3fbb0722cf8dedb3516c3edd5f.png" alt=""><h5>DIRECT</h5></div><div class="col-3 inconRight"><i class="fas fa-info-circle fa-lg"></i></div></div><div class="accountInfo row"><div class="name col-12 zoomInRightAnim">' + accounts[i]['name'] + '</div><div class="currentAcc col-12"><div class="accountName"> Current account </div><div class="info"> Available </div><div class="balance zoomInLeftAnim">€ ' + accounts[i]['balance'] + '</div></div></div></div>').appendTo(accountDisplay);
    }
  };
};
...
function init() {
  ...
  var canvasLeo = document.querySelector("canvas");

  canvasLeo.addEventListener('mousemove', handleMouseMove, false);
  canvasLeo.addEventListener('mousedown', handleMouseDown, false);
  canvasLeo.addEventListener('mouseup', handleMouseUp, false);
  canvasLeo.addEventListener('touchstart', handleTouchStart, false);
  canvasLeo.addEventListener('touchend', handleTouchEnd, false);
  canvasLeo.addEventListener('touchmove', handleTouchMove, false);
  /*
...
function handleMouseMove(canvasLeo) {
  mousePos = {
    x: canvasLeo.clientX,
    y: canvasLeo.clientY
...
let refreshData = function () {
  dataRequest.onload = whenDataLoaded;
  dataRequest.open("GET", accountsFeed, true);
